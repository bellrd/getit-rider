...
class OrderHistoryListAdapter(val mcontext: Context, val orderList: List<History>) :
    ArrayAdapter<History>(mcontext, 0, orderList) {
    init {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        val currentOrder: History = orderList[position]
        val orderId: TextView? = listItem.findViewById(R.id.order_id)
        orderId?.text = "#${currentOrder.order_id}"
        ...
        orderStatus?.text = currentOrder.last_status
        val tempColor = when (currentOrder.last_status) {
            "PLACED" -> "#214d39"
            "PICKED" -> "#c87403"
            "DELIVERED" -> "#000000"
            "CANCEL1" -> "#ba2121"
        ...
        orderAddress?.text = currentOrder.address_used
